...
# --- Final API Documentation Accuracy Closure pass: keep docs/api.md's
# raw-scorer examples honest against the live signatures, without a
# markdown parser or brittle line-number assertions. Two narrow checks:
# a denylist of keyword forms known to have been removed from a scorer's
# signature (catches the exact class of staleness this pass fixed -- a
# documented `hedging_patterns=` kwarg on score_confabulation that no
# longer exists), and a direct signature-bind check proving each raw
# scorer's documented ordinary call shape still binds against the real
# function today. Neither depends on this file's own future line numbers.

_DOCS_API_MD = Path(__file__).resolve().parent.parent / "docs" / "api.md"

# Known-removed scorer keyword forms -- add to this list (never remove an
# entry) if a future scorer-hardening pass drops another caller-supplied
# parameter, so docs/api.md is checked against every such removal, not
# just the one this pass found.
_KNOWN_REMOVED_SCORER_KEYWORDS = (
    "hedging_patterns=",
)


def test_docs_api_md_has_no_known_removed_scorer_keywords():
    text = _DOCS_API_MD.read_text()
    for removed_kw in _KNOWN_REMOVED_SCORER_KEYWORDS:
        assert removed_kw not in text, (
            f"docs/api.md documents a scorer keyword argument "
            f"({removed_kw!r}) that no longer exists on any current "
            f"scorer signature -- the example would raise TypeError."
        )


def test_docs_api_md_raw_scorer_examples_bind_against_live_signatures():
    """The four raw-scorer calls docs/api.md's "Scoring Programmatically"
    example documents, reproduced here exactly and bound (never executed)
    against the real, live function signatures -- proves the documented
    call shapes are valid today, without parsing the markdown file."""
    import inspect
    ...
    run_dir = Path("runs/20250117_143022")  # never touched -- bind only
    inspect.signature(score_mirror_loop).bind(run_dir)
    inspect.signature(score_confabulation).bind(run_dir)
    inspect.signature(score_violation_state).bind(run_dir, refusal_patterns=["I cannot help with that"])
    inspect.signature(score_echo_chamber).bind(run_dir)
